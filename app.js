...
async function loadRecipes() {
  ...
  console.log("Ricette caricate:", ALL_RECIPES.length);
}
...
function generateRecipes() {
  ...
  const filtered = ALL_RECIPES.filter(r => {
    ...
    // filtri nutrizionali
    if (r.kcal < filters.kcalMin || r.kcal > filters.kcalMax) return false;
    if (r.protein < filters.proteinMin) return false;
    if (r.carb > filters.carbMax) return false;
    if (r.fat > filters.fatMax) return false;
